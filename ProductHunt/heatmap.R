...
# Posts heat map
f.query <- dbSendQuery(connection, "SELECT DAYOFWEEK(created_at) as day, HOUR(created_at) as hour, comments_count FROM Posts GROUP BY day,hour")
...
day_hour_hm <- heatmap(mymatrix, Rowv=NA, Colv=NA, col = cm.colors(256), scale="column", margins=c(5,10))
